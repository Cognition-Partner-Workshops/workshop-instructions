...
| **License** | MIT |
| **Default Branch** | `main` |
| **Challenges** | [OtterWorks Workshop](../workshops/otterworks/README.md) — ETL modernization (cron to Airflow), framework upgrades (Java 8 to 17), language translation (Flask to FastAPI), incident response (chaos engineering), security remediation (CVE triage), API contract audit, test coverage; [Stored Procedures to Microservices + React](../demos/migration/stored-procs-to-microservices-demo.md) — a PL/pgSQL billing estate under `services/legacy-billing/`, extracted with a human-approved rule ledger and a golden-transcript parity harness; also carries the Commission Pay Oracle before-state under `services/industry-solutions/insurance/` — an Oracle Database Free OLTP/OLAP fixture with commission-rate and split-commission rules in PL/SQL packages, seed data, and PL/SQL test suites |
